...
#	move_direction = move_direction.rotated(Vector3.UP, camara.rotation.y).normalized();
	...
	parent.apply_force(move_direction * acceleration);
	
	var roll_direction = Input.get_action_strength("roll_left") - Input.get_action_strength("roll_right");
	
	parent.apply_torque(Vector3.BACK  * roll_direction * 20 * delta);
	parent.apply_torque(Vector3.RIGHT * _pitch_direction * delta);
